...
class CalendarLogic():
    ...
    def get(self):
        try:
            ...
            webhookData = WebhookData()
            webhookContent = WebhookContent()
            message = self.config["webhook"].get('event_error_message')
            body = webhookContent.createMessage(message)
    ...
    def insertLongEvent(self, event):
        try:
            ...
            webhook = Webhook()
            webhookContent = WebhookContent()
            message = self.config["webhook"].get('long_event_error_message')
            body = webhookContent.createMessage(message)
    ...
    def delete(self, event):
        try:
            ...
            message = self.config["webhook"].get('delete_message')
            body = webhookContent.createMessage(message)
            webhook.send(body)
        except Exception as e:
            print(e)
            webhook = Webhook()
            webhookContent = WebhookContent()
            message = self.config["webhook"].get('delete_error_message')
            body = webhookContent.createMessage(message)
